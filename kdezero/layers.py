...
class Layer:
    ...
    def __setattr__(self, name, value):
        if isinstance(value, (Parameter, Layer)):  # Layer も追加する
...
class Conv2d(Layer):
    def __init__(self, out_channels, kernel_size, stride=1, pad=1, nobias=False, dtype=np.float32, in_channels=None):
        super().__init__()
...
class LSTM(Layer):
    ...
    def forward(self, x):
        if self.h is None:
            ...
            u = F.tanh(self.x2u(x) + self.h2o(self.h))
